...
class SelfMultiheadAttentionwithLoRA(nn.Module):
    def __init__(
        self,
        embed_dim,
        num_heads,
        ...
        self.lora_q_A = nn.Parameter(torch.zeros(lora_r, embed_dim))
        self.lora_q_B = nn.Parameter(torch.zeros(embed_dim, lora_r))
        nn.init.normal_(self.lora_q_A, std=0.02)
        nn.init.zeros_(self.lora_q_B)
        self.lora_k_A = nn.Parameter(torch.zeros(lora_r, embed_dim))
        self.lora_k_B = nn.Parameter(torch.zeros(embed_dim, lora_r))
        nn.init.normal_(self.lora_k_A, std=0.02)
        nn.init.zeros_(self.lora_k_B)
        self.lora_v_A = nn.Parameter(torch.zeros(lora_r, embed_dim))
        self.lora_v_B = nn.Parameter(torch.zeros(embed_dim, lora_r))
        nn.init.normal_(self.lora_v_A, std=0.02)
        nn.init.zeros_(self.lora_v_B)
        self.lora_o_A = nn.Parameter(torch.zeros(lora_r, embed_dim))
        self.lora_o_B = nn.Parameter(torch.zeros(embed_dim, lora_r))
        nn.init.normal_(self.lora_o_A, std=0.02)
        nn.init.zeros_(self.lora_o_B)
    ...
    def forward(
        ...
        # NOTE: LoRA
        q = q + (query @ self.lora_q_A.T @ self.lora_q_B.T) * self.scaling
        k = k + (query @ self.lora_k_A.T @ self.lora_k_B.T) * self.scaling
        v = v + (query @ self.lora_v_A.T @ self.lora_v_B.T) * self.scaling
        ...
        o = self.out_proj(o)
        # NOTE: LoRA
        o = o + (flag_o @ self.lora_o_A.T @ self.lora_o_B.T) * self.scaling
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.self_attn = SelfMultiheadAttentionwithLoRA(
            self.embed_dim,
            attention_heads,
            dropout=attention_dropout,
            lora_r=lora_r,  # 
            lora_alpha=lora_alpha,  #
        ) if use_lora else SelfMultiheadAttention(
            self.embed_dim,
            attention_heads,
            dropout=attention_dropout,
        )
