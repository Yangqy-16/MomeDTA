...
class TargetRepresentationSelf(nn.Module):
    def __init__(self, block_num, embedding_num): #vocab_size, 
        super().__init__()
        # self.embed = nn.Embedding(vocab_size, embedding_num, padding_idx=0)
        self.pre_linear = nn.Linear(embedding_num, embedding_num // 3)
        embedding_num = embedding_num // 3
...
class GraphNet(nn.Module):
    @configurable
    def __init__(self, cfg, #drug_dim, common_dim, drug_max_len,
                 block_num=3, vocab_protein_size=26, embedding_size=128, filter_num=32, out_dim=1,
                 predict=False):#
        super().__init__()
        self.cfg = cfg
        drug_dim = cfg.MODEL.DRUG_2D_DIM
        common_dim = cfg.MODEL.PROJECTION_DIM
        drug_max_len = cfg.DATASET.DRUG_MAX_LEN

        if cfg.MODEL.PROT_2D_MODEL == 'gearnet':
            self.protein_encoder = TargetRepresentationSelf(block_num, cfg.MODEL.PROT_2D_DIM) # CNN_1D(input_dim=3072, hidden_dim=96, max_len=1000)
        else:
            self.protein_encoder = ProtGNN(out_channels=128) #TargetRepresentation(block_num, vocab_protein_size, embedding_size) #  #Downsample_CNN(input_dim=3072, output_dim=96)  # 
        
        if cfg.MODEL.DRUG_2D_MODEL == 'molclr':
            self.ligand_encoder = CNN_1D(input_dim=drug_dim, hidden_dim=96, max_len=drug_max_len)
        else:
            self.ligand_encoder = GraphDenseNet(num_input_features=22, out_dim=128, block_config=[8, 8, 8], bn_sizes=[2, 2, 2])  # filter_num*3
        
        self.predict = predict
        if predict:
            self.classifier = nn.Sequential(
                nn.Linear(128 * 2, 1024), #filter_num * 3
                nn.ReLU(),
                nn.Dropout(0.1),
                nn.Linear(1024, 1024),
                nn.ReLU(),
    ...
    def forward(self, data): #drug: Data, prot: Data
        # target = data['prot2d_embed'] #.target
        # if self.cfg.MODEL.PROT_2D_MODEL == 'gearnet':
        #     _, protein_x = self.protein_encoder(target)
        # else:
        protein_x = self.protein_encoder(data['prot2d_embed']) #
        
        ligand_x = self.ligand_encoder(data['drug2d_embed']) #
        # ligand_x = torch.mean(ligand_x, dim=1)

        feat = torch.cat([protein_x, ligand_x], dim=-1)
        if self.predict:
