...
def batch_collate_fn(samples: list[dict[str, torch.Tensor]]):
    """
    ...
    for k in samples[0].keys():
        if k == 'src_coord':
            ...
            v = pad_2d([torch.tensor(s[k]).long() for s in samples], pad_idx=0.0) #padding_idx
        elif k == 'src_distance':
            v = pad_2d([torch.tensor(s[k]).float() for s in samples], pad_idx=0.0)
        elif k == 'src_tokens':
            v = pad_1d_tokens([torch.tensor(s[k]).long() for s in samples], pad_idx=0.0)  #padding_idx
        batch[k] = v
    # try:
